...
function count_rows($clauses = array(), $keywords = null, $exact = false) {
	...
	$sth = $dbh->prepare('SELECT COUNT(file_id) FROM '.$config['table_prefix'].'pictures
	LEFT JOIN forum_users ON '.$config['table_prefix'].'pictures.file_userid = forum_users.user_id'
...
function delete_pictures($ids, $upload_dir) {
	global $dbh, $config;

	$files = array();

	$sth = $dbh->prepare('SELECT file_name FROM '.$config['table_prefix'].'pictures WHERE file_id IN ('.placeholders('?', sizeof($ids)).')');
	...
	if($data) {
		...
		if(count(array_filter($files, 'file_exists')) == 0) {
			$sth = $dbh->prepare('DELETE FROM '.$config['table_prefix'].'pictures WHERE file_id IN ('.placeholders('?', sizeof($ids)).')');
			$sth->execute(array_values($ids));
			unset($sth);
