	...
	<div class="row">
		<div class="col-sm-9 col-sm-push-3 col-md-7 col-md-push-4 col-lg-6 col-lg-push-4">
			<ol class="breadcrumb">
			  ...
			  <li class="active"><?php echo $lang_avatars["manage_avatars"];?></li>
			</ol>
			<div class="panel panel-default">
			<div class="panel-body">
			<?php if( $total > 0 ){ ?>
			<form class="form-inline form-search" method="get" action="http://forum.prog/adm/avatars.php">
			  <div class="form-group">
				<input type="text" name="search" id="search" value="<?php echo display( $search["keywords"] );?>" class="form-control input-sm" placeholder="<?php echo $lang_avatars["search"];?>" />
			  </div>
			  <div class="form-group">
			  ...
			  <div class="form-group">
				<label for="exact"><input type="checkbox" name="exact" id="exact" value="1" <?php if( !empty($search["keywords"]) && $search["exact"] ){ ?>checked <?php } ?>/> <?php echo $lang_avatars["exact"];?></label>
			  </div>
			  <button class="btn btn-default btn-sm" type="submit"><span class="glyphicon glyphicon-search"></span></button>
			  <?php if( !empty($search["keywords"]) ){ ?><a class="btn btn-primary btn-sm pull-right hidden-xs" href="http://forum.prog/adm/avatars.php?page=1"><b><?php echo $lang_avatars["back"];?></b></a><?php } ?>
			</form>

			<?php if( $nb > 0 ){ ?>
			<?php if( !empty($search["keywords"]) ){ ?>
			<div class="alert alert-success text-center"><?php echo $lang_avatars["search_results"];?></div>
			<?php } ?>
			<div class="row">
			...
			<form method="post" action="http://forum.prog/adm/avatars.php">
			<input type="hidden" name="token" value="<?php echo $token;?>" />
			<div class="form-group">
			<button class="action-submit btn btn-danger" data-toggle="tooltip" title="<?php echo $lang_avatars["delete_selection"];?>"><?php echo $lang_avatars["delete"];?></button>
			</div>
			...
			<th><?php echo $lang_avatars["weight"];?></th>
			<th><?php echo $lang_avatars["username"];?></th>
			<th><?php echo $lang_avatars["date"];?></th>
			</tr>
			...
			<script>
			var text_enlarge_image = '<?php echo escape_quotes( $lang_avatars["click_to_enlarge"] );?>';
			...
			<div class="alert alert-warning text-center"><?php echo $lang_avatars["search_no_result"];?></div>
			<?php } ?>
			<?php }else{ ?>
			<div class="alert alert-danger text-center"><?php echo $lang_avatars["no_data"];?></div>
			<?php } ?>
			</div>
			</div>
		</div>
		...
		<div class="col-sm-3 col-sm-pull-9 col-md-3 col-md-pull-6 col-lg-2 col-lg-pull-4">
			...
			<a class="list-group-item" href="http://forum.prog/adm/banlist.php"><?php echo $lang_avatars["banlist"];?></a>
			<a class="list-group-item active" href="http://forum.prog/adm/avatars.php"><?php echo $lang_avatars["avatars"];?></a>
			</div>
		</div>
	</div>
</div>
