...
include $root_path.'includes/functions/viewtopic.php';
include $root_path.$lang_path.'viewtopic.php';
...
if($topic['topic_invisible'] == 1 && $user->data['user_rank'] < ADMIN)
...
if($user->data['user_rank'] >= $topic['forum_auth_view']) {
	if(isset($_GET['get-post']) && ctype_digit($_GET['get-post'])) {
		if($user->data['user_rank'] < $topic['forum_auth_edit'])
		...
		if($user->data['user_id'] != $post['post_userid'] && $user->data['user_rank'] < ADMIN)
	...
	if($user->data['user_rank'] >= $topic['forum_auth_reply']) {
